...
export default function CouplesInsights({ personal }: Props) {
  const [open, setOpen] = useState(false);
  const { partnerRecords, opponentRecords, bestPartner, toughestOpponent, unbeatenStreak, standing } = personal;

  if (standing.matchesPlayed < 1) return null;
        ...
        <Card className="bg-gradient-card border-border/40 p-3 shadow-card mt-2 space-y-4">
          ...
          <Section icon={<TrendingUp className="h-3.5 w-3.5" />} title="שערים">
            <div className="grid grid-cols-3 gap-2">
              <MiniStat label="בעד" value={standing.goalsFor} />
              <MiniStat label="נגד" value={standing.goalsAgainst} />
              <MiniStat label="הפרש" value={`${standing.goalDiff > 0 ? '+' : ''}${standing.goalDiff}`} />
            </div>
          </Section>
                ...
                {partnerRecords.map(pr => (
                  <div key={pr.partner.id} className="flex items-center justify-between bg-gaming-surface/40 rounded-md px-2.5 py-1.5 border border-border/30 text-xs">
                    ...
                    <span className="text-muted-foreground">
                      {pr.matchWins}נ {pr.matchDraws}ת {pr.matchLosses}ה · {pr.roundWins} סיבובים
                    </span>
                  </div>
                ))}
              </div>
          ...
          {toughestOpponent && toughestOpponent.matchesPlayed >= 1 && (
            <Section icon={<Swords className="h-3.5 w-3.5" />} title="יריבים">
              <div className="space-y-1.5">
                {opponentRecords
                  .sort((a, b) => b.matchesPlayed - a.matchesPlayed || b.matchLosses - a.matchLosses)
                  .map(or => (
                    <div key={or.opponent.id} className="flex items-center justify-between bg-gaming-surface/40 rounded-md px-2.5 py-1.5 border border-border/30 text-xs">
                      ...
                      <span className="text-muted-foreground">
                        {or.matchWins}נ {or.matchDraws}ת {or.matchLosses}ה
                      </span>
                    </div>
                  ))}
              </div>
